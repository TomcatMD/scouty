  ...
  class TestScrapers < Minitest::Test
    ...
    def test_from_configs
      configs = [
        ...
        mock(source: "justjoin.it", params: {}),
        mock(source: "nofluffjobs.com", params: { "category" => ["backend"] }),
      ...
      assert_equal ["backend"], result[2].category
    ...
    def test_init_just_join_it
      result = Scrapers.init("justjoin.it", webcache:)

      assert_instance_of Scrapers::JustJoinIT, result
    end

    def test_init_no_fluff_jobs
      result = Scrapers.init("nofluffjobs.com", webcache:, params: { "category" => ["backend"] })

      assert_instance_of Scrapers::NoFluffJobs, result
      assert_equal ["backend"], result.category
    end
